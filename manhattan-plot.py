...
def manhattan(fhs, columns, image_path, no_log, colors, sep, title, lines, ymax):
    ...
    if ymax is not None:
        plt.ylim(ymax=ymax)
    plt.xticks(
        [c[1] for c in xs_by_chr], [c[0] for c in xs_by_chr], rotation=-90, size=8.5
    )
...
def main():
    ...
    p.add_option(
        "--no-log",
        dest="no_log",
        help="don't do -log10(p) on the value",
        action="store_true",
        default=False,
    )
    p.add_option(
        "--cols",
        dest="cols",
        help="zero-based column indexes to get"
        " chr, position, p-value respectively e.g. %default",
        default="0,1,2",
    )
    p.add_option(
        "--colors", dest="colors", help="cycle through these colors", default="bk"
    )
    p.add_option(
        "--image",
        dest="image",
        help="save the image to this file. e.g. %default",
        default="manhattan.png",
    )
    p.add_option("--title", help="title for the image.", default=None, dest="title")
    p.add_option(
        "--ymax", help="max (logged) y-value for plot", dest="ymax", type="float"
    )
    p.add_option(
        "--sep", help="data separator, default is [tab]", default="\t", dest="sep"
    )
    p.add_option(
        "--lines",
        default=False,
